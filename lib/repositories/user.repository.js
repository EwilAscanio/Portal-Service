...
export async function create({ login, name, email, passwordHash, roleId, status = "Activo" }) {
  const { rows } = await query(
    `
    INSERT INTO "user" (login, name, email, password_hash, role_id, status)
    VALUES ($1, $2, $3, $4, $5, $6)
    RETURNING id
    `,
...
export async function setStatus(id, status) {
  await query(`UPDATE "user" SET status = $2 WHERE id = $1`, [id, status]);
  return findById(id);
}

export async function touchLastAccess(id) {
  await query(`UPDATE "user" SET last_access_at = now() WHERE id = $1`, [id]);
}
